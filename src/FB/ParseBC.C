...
void ParseBC::chkBCconsistency(const int kos, const CompoList* cmp)
{
  ...
  else if (kos == SOLID_CONDUCTION)
  {
    for (int n=1; n<=NoCompo; n++)
    {
      if ( cmp[n].isVBC() ) 
      {
        Hostonly_ stamped_printf("\t'KindOfSolver' is SOLID_CONDUCTION, but '/BCtable/Boundary' has velocity boundary condition.\n");
        Exit(0);
      }
    }
  }
...
void ParseBC::getIbcPeriodic(const string label_base, const int n, CompoList* cmp)
{
  ...
  if ( !(tpCntl->getInspectedValue(label, str )) )
  {
	  printf("\tParsing error : fail to get 'upstream_direction' in '/BCtable/Boundary > Periodic'\n");
	  Exit(0);
  }
  ...
  {
    stamped_printf("\tParsing error : Invalid direction in '/BCtable/Boundary > Periodic'\n");
    Exit(0);
  }
  ...
  if ( !(tpCntl->getInspectedValue(label, ct )) )
  {
    printf("\tParsing error : Invalid value of 'Pressure difference' in '/BCtable/Boundary > Periodic'\n");
    Exit(0);
  }
  else
  {
...
// #################################################################
/**
 * @brief 内部のポリゴン境界のパラメータを取得する
 * @param [in]  label_base ラベルディレクトリ
 * @param [in]  n          コンポーネントリストの格納番号
 * @param [out] cmp        CompoList
 */
void ParseBC::getIbcPolygon(const string label_base, const int n, CompoList* cmp)
{
...
void ParseBC::getIbcSpecVel(const string label_base, const int n, CompoList* cmp)
{
  ...
  // 速度パラメータの読み込み
  getVelocity(label_base, cmp[n].get_V_Profile(), cmp[n].ca, "Boundary", cmp[n].isPolicy_Massflow());
...
// #################################################################
// CompoListに境界条件の情報を設定する
/**
 * @note 最初にBCの情報を登録，その後IDの情報を登録
 * @note パラメータファイルからBCのidをパースし，cmpに保持する
 * @note 格納番号は1からスタート
 */
void ParseBC::loadBCs(Control* C, MediumList* mat, CompoList* cmp)
{ 
  string str, label;
  string label_base, label_ename, label_leaf;
  
  //
  // 境界条件のCompoListへの登録  NoBC = リストアップした境界の数
  // setControlVars()で配列などを確保済み
  //
  ...
  */
  ...
  for (int k=1; k<=NoBC; k++)
  {
    ...
    if ( !strcasecmp( str.c_str(), "inner" ) )
    {
      cmp[m].kind_inout = CompoList::kind_inner;
    }
    else if ( !strcasecmp( str.c_str(), "outer" ) )
    {
    ...
    if ( cmp[m].kind_inout == CompoList::kind_inner )
    {
    ...
    if ( cmp[m].kind_inout == CompoList::kind_inner ) // 内部BCの処理
    {
      int tp = cmp[m].getType();
      
      if      ( tp == OBSTACLE )      getIbcPolygon(label_leaf, m, cmp);
    ...
    {
      switch ( BaseBc[odr_outer].getClass() )
      {
        case OBC_WALL:
          getObcWall(label_leaf, odr_outer);
          break;
          
        case OBC_OUTFLOW:
          getObcOutflow(label_leaf, odr_outer);
          break;
          
        case OBC_SPEC_VEL:
          getObcSpecVH(label_leaf, odr_outer);
          break;
          
        case OBC_TRC_FREE:
          getObcTrcfree(label_leaf, odr_outer);
          break;
          
        case OBC_FAR_FIELD:
          getObcFarField(label_leaf, odr_outer);
          break;
          
        case OBC_PERIODIC:
          getObcPeriodic(label_leaf, odr_outer);
          break;
...
void ParseBC::printCompoSummary(FILE* fp, CompoList* cmp, const int basicEq)
{
  ...
  {
    fprintf(fp,"\t  No :  # of Faces/Cells       State   Kind   Phase                     Label : Class                                 Medium\n");
    fprintf(fp,"\t------------------------------------------------------------------------------------------------------------------------------\n");
  ...
  else
  {
    if ( KindOfSolver == FLOW_ONLY )
    {
      ...
      for (int i=1; i<=NoCompo; i++)
      {
        ...
        if ( cmp[i].getType() != 0 ) {
          if ( cmp[i].kind_inout == CompoList::kind_inner ) {
            fprintf(fp, " Inner ");
          } else if ( cmp[i].kind_inout == CompoList::kind_outer ) {
            fprintf(fp, " Outer ");
          }
        }
        else {
...
void ParseBC::setControlVars(Control* Cref)
{
  ...
  for (int k=1; k<=NoBC; k++)
  {
    ...
    {
      Hostonly_ stamped_printf("\tInvalid keyword : '%s'\n", label.c_str());
      Exit(0);
    }
  }
  
  
  // リストアップされた外部境界条件数でコンテナをインスタンス
  NoBaseBC = Cref->NoBCouter;
